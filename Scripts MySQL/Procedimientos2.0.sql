...
-- -----------------------------------------------------
-- PROCEDURE sp_cambiar_Datos_completos_oferta_laboral
-- Procedimiento para cambiar todos los datos de una oferta laboral de una oferta laboral, es el que se usa en la aplicacion
-- -----------------------------------------------------

DROP PROCEDURE IF EXISTS sp_cambiar_Datos_completos_oferta_laboral;
DELIMITER $$

CREATE PROCEDURE sp_cambiar_Datos_completos_oferta_laboral (no_oferta_cambio BIGINT,nuevo_estado VARCHAR(15), nuevo_salario INT,
nueva_exp_requerida INT, nueva_area VARCHAR(40), nuevo_tipo_contrato VARCHAR(40)
)
BEGIN
    
    IF nuevo_estado IS NOT NULL THEN
		CALL sp_cambiar_Estado_oferta_laboral(no_oferta_cambio, nuevo_estado);
	END IF;
	IF nuevo_salario IS NOT NULL THEN
		CALL sp_cambiar_Salario_oferta_laboral(no_oferta_cambio, nuevo_salario);
	END IF;
	IF nueva_exp_requerida IS NOT NULL THEN
		CALL sp_cambiar_ExpRequerida_oferta_laboral(no_oferta_cambio, nueva_exp_requerida);
	END IF;
	IF nueva_area IS NOT NULL THEN
		CALL sp_cambiar_Area_oferta_laboral(no_oferta_cambio, nueva_area);
	END IF;
    IF nuevo_tipo_contrato IS NOT NULL THEN
		CALL sp_cambiar_TipoContrato_oferta_laboral(no_oferta_cambio, nuevo_tipo_contrato);
	END IF;
END $$
...
-- -----------------------------------------------------
-- PROCEDURE  sp_generar_usuario_empresa
-- Procedimiento para crear un usuario por cada egresado insertado en la base de datos
-- -----------------------------------------------------

DROP PROCEDURE IF EXISTS sp_generar_usuario_empresa;
DELIMITER $$
CREATE PROCEDURE sp_generar_usuario_empresa (usuario VARCHAR(40), contrasena VARCHAR (40))
BEGIN

	
	
	SET @crear_usuario = CONCAT('CREATE USER ''', usuario, '''@''localhost'' IDENTIFIED BY ''', contrasena, ''';');
    PREPARE stmt FROM @crear_usuario;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;

    SET @asignar_rol = CONCAT('GRANT  Empresa TO ''', usuario, '''@''localhost'';');
	PREPARE stmt2 FROM @asignar_rol;
	EXECUTE stmt2;
	DEALLOCATE PREPARE stmt2;
    
    SET @default_rol = CONCAT('SET DEFAULT ROLE Empresa TO ''', usuario, '''@''localhost'';');
	PREPARE stmt3 FROM @default_rol;
	EXECUTE stmt3;
	DEALLOCATE PREPARE stmt3;
END $$
