...
BEGIN
    ...
    SET correo_buscar_mod = LEFT (correo_buscar, LENGTH(correo_buscar) -8 );
...
SELECT f_encontrar_NIT('maria.gomez');
...
BEGIN
	DECLARE correo_buscar_mod VARCHAR (50);
    SET correo_buscar_mod = LEFT (correo_buscar, LENGTH(correo_buscar) -6 );
    RETURN (SELECT egresado.cedula FROM egresado 
    WHERE egresado.CorreoElectronico  LIKE CONCAT(correo_buscar_mod, '%')) ;
...
BEGIN
   DECLARE Nombre_usuario VARCHAR (50) ;
    ...
    GRANT Empresa TO Nombre_usuario@'localhost';
	SET DEFAULT ROLE Empresa TO Nombre_usuario@'localhost';
    
    SET @asignar_rol = CONCAT('GRANT Empresa TO ''', Nombre_usuario, '''@''localhost'';');
	PREPARE stmt2 FROM @asignar_rol;
	EXECUTE stmt2;
	DEALLOCATE PREPARE stmt2;
    
    SET @default_rol = CONCAT('SET DEFAULT ROLE Empresa TO ''', Nombre_usuario, '''@''localhost'';');
...
-- Call sp_generar_usuario('Zorro.Zamora@unal.edu.co', 'isabelmendoza2204..');
